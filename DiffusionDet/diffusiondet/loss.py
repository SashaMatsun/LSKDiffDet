...
class SetCriterionDynamicK(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=False):
        ...
        if self.use_focal or self.use_fed_loss:
            ...
            if self.use_fed_loss:
                ...
                K = self.num_classes
                N = src_logits.shape[0]
                # weight = torch.tensor([0.0338, 0.2059, 2.3867, 0.4155, 1.8008, 2.279 , 0.6011, 0.0332, 0.0056, 1.8635, 0.5457, 2.804 , 1.7013, 0.1452, 0.1786]).to('cuda:0')
                # weight = weight.expand(N, K).float()
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(pred_box_list) != 0:
            ...
            # loss_bbox = F.smooth_l1_loss(src_boxes_norm, box_cxcywh_to_xyxy(target_boxes), reduction='none')
